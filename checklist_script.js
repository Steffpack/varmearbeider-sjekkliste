...
async function generatePDF(dato, områder, fagarbeidere, sjekkpunkter, signatureImage, imageUrls = [], submissionTime) {
...
async function loadCustomFont(doc) {
  ...
  doc.roundedRect(10, 30, 190, 28, 3, 3, "F");
  doc.setFontSize(11);
  doc.setTextColor("#000");
  doc.setFont("helvetica", "normal");
  doc.text(`Dato: ${dato}`, 15, 38);
  doc.text(`Arbeidstid: 07:00 - 18:30`, 15, 44);
  doc.text(`Tidspunkt for innsending: ${submissionTime}`, 15, 56);
  doc.text(`Områder: ${områder.length ? områder.join(", ") : "Ingen valgt"}`, 15, 50);

  // === Fagarbeidere ===
  doc.setFillColor(habGray);
  doc.roundedRect(10, 65, 190, 100, 3, 3, "F");
  doc.setFontSize(13);
  doc.setTextColor(habBlue);
  doc.text("Fagarbeidere med kurs", 15, 73);
  doc.setFontSize(10);
  doc.setTextColor("#000");

  let col1 = 15, col2 = 110;
  let y = 82;
  const half = Math.ceil(fagarbeidere.length / 2);
  fagarbeidere.forEach((name, i) => {
    if (i < half) {
      doc.text(name, col1, y);
      y += 6;
    } else {
      doc.text(name, col2, y - (half * 6));
      y += 6;
    }
  });
...
const pdfBlob = await generatePDF(datoInput.value, områder, fagarbeidere, sjekkpunkter, canvas.toDataURL(), imageUrls, submissionTime);
...
showToast(`✅ Sjekkliste sendt! Tidspunkt: ${submissionTime}`);
